---
name: defect-metrics-report
description: An analytical report on defect and quality metrics — trends and interpretation, not a raw list of bugs: defect density by component, distribution by severity/priority, reopen rate, aging of open defects, MTTR/lead time, arrival vs closure, Defect Removal Efficiency and escape rate, share of regressions, trends over time, with a "what this means and what to do" conclusion for each metric. Use when asked for bug metrics/statistics, a defect report, quality trends, to count how many bugs there are and where they concentrate, DRE or escape rate, defect analytics for a period/sprint/release, or to understand the quality health of a project by the numbers. Works with any tracker (Jira/YouTrack/GitHub Issues/Linear) via an available MCP tool, CSV export or a tabular source; does not invent data — unavailable metrics are flagged explicitly. This is NOT `bug-triage` (which prioritizes a specific bug queue) — here it is aggregated analytics and trends over a body of defects. Trigger even without the word "metrics", for example "where are bugs piling up for us", "how long does a bug live before a fix", "are there many reopens".
