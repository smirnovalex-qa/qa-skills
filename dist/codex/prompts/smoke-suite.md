---
description: Проектирует и пишет smoke/sanity-набор для быстрой проверки после деплоя — только критические пути (login, ключевой бизнес-флоу, платёж/checkout, основные CRUD, health-эндпоинты), быстрый, стабильный, безопасный для прод/стейджа, с чёткими PASS/FAIL; затем реально запускает набор и показывает вывод.
